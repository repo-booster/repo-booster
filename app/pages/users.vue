...
const input = ref<{ input: HTMLInputElement } | null>(null)
...
const defaultStatuses = ref(['active', 'inactive', 'pending'])

const defaultLocations = computed(() =>
  users.value.reduce((acc, user) => {
    if (user.location && !acc.includes(user.location)) {
...
// Define the missing `onSelect` method
